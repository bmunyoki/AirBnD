...
class Office extends Model
{
    ...
    public function scopeNearestTo(Builder $builder, $lat, $lng) {
        /*return $builder->select()
            ->selectRaw(
                'SQRT(POW(69.1 * (lat - ?), 2) + POW(69.1 * (? - lng) * COS(lat / 57.3), 2)) AS distance', [$lat, $lng]
            )->orderBy('distance'); */

        // To remove the added distance attribute to the data/response, use below code
        return $builder->select()
            ->orderByRaw(
                'SQRT(POW(69.1 * (lat - ?), 2) + POW(69.1 * (? - lng) * COS(lat / 57.3), 2))', [$lat, $lng]
            );
    }
}
